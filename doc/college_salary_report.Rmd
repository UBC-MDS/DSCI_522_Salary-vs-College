...
```{r echo = FALSE, results = "show"}
df <- read_csv("results/anova_results/region_tukey_results.csv") %>% filter(adj.p.value < 0.05) %>%
   select(-term)
...
```{r echo = FALSE, results = "show"}
df <- head(read_csv("results/anova_results/school_type_tukey_results.csv")) %>%  filter(adj.p.value < 0.05) %>%
  select(-term)
