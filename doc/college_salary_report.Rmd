...
**Figure 1**: Salary data is ordered by starting salary
![Figure 1](../results/degree_vs_salary_by_start.png)

**Figure 2**: Salary data is ordered by mid-career salary
...
df <- read_csv("results/anova_results/region_anova_results.csv")
knitr::kable(df, digits = c(NA, 2, 15), col.names = c("Salary Type", "F Statistic", "p-value"))
```

**Table 1** above shows there was a statistical difference in salary, all salary types examined, amongst the different regions, with a p-value < 0.05. Next we performed a Tukey's post-hoc test to see which region was responsible for the difference in salary. Summary is presented in Table 2 below.

```{r echo = FALSE, results = "show"}
df <- head(read_csv("results/anova_results/region_tukey_results.csv") )%>% 
  select(-term)

knitr::kable(df, col.names = c("Salary Type", "Region Comparison", "adjusted p-value", "Interpretation"))
```
...
```{r echo = FALSE, results = "show"}
df <- read_csv("results/anova_results/school_type_anova_results.csv")
knitr::kable(df, digits = c(NA, 2, 30), col.names = c("Salary Type", "F Statistic", "p-value"))
```
...
```{r echo = FALSE, results = "show"}
df <- head(read_csv("results/anova_results/school_type_tukey_results.csv")) %>% 
  select(-term)
...
One of the major limitations of our analysis was that for the dataset we chose, the individual datapoints for each salary were not given. Instead, the medians for each salary type were pre-calculated and tabulated into this dataset. This meant that:
* We had to take the dataset as the absolute truth since we could not calculate the medians ourselves, and
* We could not see the spread of the datapoints (ie. summary statistics, or visualizing the data as a boxplot).
