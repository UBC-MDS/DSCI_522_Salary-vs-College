...
df$Salary_Type <- gsub('\\_',' ', df$Salary_Type)
...
**Table 3**
```{r echo = FALSE, results = "show", message=FALSE}
#create a table for mid-career median salary BY REGION tukey tests only
df <- read_csv("results/anova_results/region_tukey_results.csv") %>% 
  select(-term, -flag) %>% 
  filter(salary_type == "Mid_Career_Median_Salary") %>% 
  separate(comparison, into = c("row", "col"), sep = "-")
...
**Table 5**
```{r echo = FALSE, results = "show", message=FALSE}
#create a table for starting career median salary BY SCHOOL TYPE tukey tests
...
#create empty matrix to enter p-values
table <- matrix(nrow = 5, ncol = 5, dimnames = list(c("Ivy League", "Engineering", "Liberal Arts", "Party", "State"),
                                                    c("Ivy League", "Engineering", "Liberal Arts", "Party", "State")))
...
**Table 6**
```{r echo = FALSE, results = "show", message=FALSE}
#create a table for mid-career median salary BY SCHOOL TYPE tukey tests only
df <- read_csv("results/anova_results/school_type_tukey_results.csv") %>% 
  select(-term, -flag) %>% 
  filter(salary_type == "Mid_Career_Median_Salary") %>% 
  separate(comparison, into = c("row", "col"), sep = "-")
